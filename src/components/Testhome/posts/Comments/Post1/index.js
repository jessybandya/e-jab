import { Divider, Input, Comment, List } from "antd";
...
//                 {/* <Comment
//             key={commentId}
//             author={profileUserData?.firstName}
//             avatar={`${profileUserData?.photoURL}`}
//             content={comment}
  ...
  useEffect(() => {
    ...
    return (
        <div>
                      ...
                      {/* <Comment
            key={commentId}
            author={`${profileUserData?.firstName} ${profileUserData?.lastName}`}
            avatar={`${profileUserData?.photoURL}`}
            content={comment}
            datetime={moment(timestamp)
              .startOf("seconds")
          ...
          <div style={{display: "flex",marginTop:0,justifyContent:"space-between",padding:5,border: "2px solid #C5C5C5",borderRadius:5,backgroundColor:"white"}}>
